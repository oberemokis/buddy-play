...
- `dts: false` в конфиге MF — архивы типов MF не создаются. Типы ремоутов лежат в `apps/web/host/src/remotes.d.ts`, который **автогенерируется** скриптом `scripts/gen-remotes.ts` (`pnpm gen:remotes`, привязан к `predev`/`prebuild`) из манифеста в `@sync/config`. Руками файл не редактировать.
