# @sync/stores — Pinia-сторы (синглтон MFE)
...
- Любой стор, определённый здесь, автоматически разделяется между всеми MFE, потому что `@sync/stores` является синглтоном Module Federation.
...
3. Потребители: `import { useXStore } from "@sync/stores"` в любом приложении или remote.
